    ...
    try {
        ...
        if (body.responses && body.score) {
            ...
            if (!templateId) {
                ...
                const template = await AssessmentTemplate.findOne().sort({ createdAt: -1 });
                templateId = template?._id;
            }
            ...
            return NextResponse.json({ sessionId: session._id, message: "Session saved" });
        }
        ...
        const totalQuestionsSetting = await Settings.findOne({ key: SETTINGS_KEYS.TOTAL_QUESTIONS });
        const maxQuestions = totalQuestionsSetting?.value || 30;

        const fullTemplate = await AssessmentTemplate.findById(template._id).populate("questions");
