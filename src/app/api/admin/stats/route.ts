...
export async function GET() {
    try {
        ...
        const [
            ...
            Session.countDocuments({ status: "completed", completed_at: { $gte: startOfDay } }),
            Session.countDocuments({ status: "completed", completed_at: { $gte: startOfWeek } }),
            Session.countDocuments({ status: "completed", completed_at: { $gte: startOfMonth } }),
        ...
        return NextResponse.json({
            ...
            liveAssessment: liveTemplate
                ? { name: liveTemplate.name, questionCount: liveTemplate.questions?.length || 0 }
                : null,
