...
export async function GET() {
    try {
        ...
        return NextResponse.json({ error: "Failed to fetch DCAS configuration" }, { status: 500 });
    }
}
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!body.D || !body.C || !body.A || !body.S) {
            return NextResponse.json({ error: "All 4 DCAS types are required." }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: "Failed to update DCAS configuration" }, { status: 500 });
    }
}
